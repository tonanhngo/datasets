...
class WikipediaConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, date=None, languages=None, **kwargs):
    """BuilderConfig for Wikipedia.

    Args:
      date: string, date of the Wikipedia dump in YYYYMMDD format. A list of
        available dates can be found at https://dumps.wikimedia.org/enwiki/.
      languages: `<list>(string)` (optional), the list of language codes to
        create splits for. If None, all languages will be used.
      **kwargs: keyword arguments forwarded to super.
    """
    super(WikipediaConfig, self).__init__(
        name="{0}.{1}".format(
            date, "-".join(languages) if languages else "all"),
        description=(
            "Wikipedia dataset split by language, parsed from %s dump" % date),
        **kwargs)
    self.date = date
    self.languages = languages or _WIKIPEDIA_LANGUAGES
...
class Wikipedia(tfds.core.BeamBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    languages = self._builder_config.languages

    info_urls = {}
    for lang in languages:
      info_urls["%s_info" % lang] = _base_url(lang) + _INFO_FILE

    info_filepaths = dl_manager.download_and_extract(info_urls)

    xml_urls = collections.defaultdict(list)
    total_bytes = collections.defaultdict(int)
    for lang in languages:
      with tf.io.gfile.GFile(info_filepaths["%s_info" % lang]) as f:
      ...
      for fname, info in multistream_dump_info["files"].items():
        if ".xml" not in fname:
          continue
        total_bytes[lang] += info["size"]
        xml_urls[lang].append(_base_url(lang) + fname)

    downloaded_files = dl_manager.download_and_extract(xml_urls)

    return (
        tfds.core.SplitGenerator(  # pylint:disable=g-complex-comprehension
            name=lang,
            num_shards=int(
                math.ceil(total_bytes[lang] / (4 * 2 ** 30))),  # max 4GB
            gen_kwargs={"filepaths": downloaded_files[lang],
                        "language": lang})
        for lang in languages)
