...
def _write_binary_matrix(filename, array):
  """Writes array as binary formatted matrix to the file."""
  with tf.gfile.GFile(filename, "wb") as f:
    ...
    if array.dtype.str == "<i4":
      ...
      f.write(np.int32(507333716).tobytes())
    elif array.dtype.str == "|u1":
      # Magic code for uint8.
      f.write(np.int32(507333717).tobytes())
    else:
      raise ValueError("Array data type '%s' not supported." % array.dtype.str)

    # Next, we specify the number of dimensions of the array to be stored as a
    # 32 bit integer.
    f.write(np.int32(array.ndim).tobytes())
    ...
    f.write(np.int32(shape).tobytes())

    # Finally, the data is written as a C-contiguous matrix.
