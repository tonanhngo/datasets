...
_TRAINING_URL_TEMPLATE = ("https://cs.nyu.edu/~ylclab/data/norb-v1.0-small/"
                 "smallnorb-5x46789x9x18x6x2x96x96-training-{type}.mat.gz")
_TESTING_URL_TEMPLATE = ("https://cs.nyu.edu/~ylclab/data/norb-v1.0-small/"
...
def _load_chunk(dat_path, cat_path, info_path):
  ...
  # There are two images for each record. We only consider the first one.
  dat_array = dat_array[:, 0]
  # Even if the image is gray scale, we need to add an extra dimension for
  # channels.
  ...
  # To modify this array, we need to make a copy.
...
def _read_binary_matrix(filename):
  ...
  with tf.gfile.GFile(filename, "rb") as f:
    ...
    # stored array.
    ndim = int(np.frombuffer(s, dtype="int32", count=1, offset=4))

    # The next ndim x 4 bytes contain the shape of the array in int32.
    dims = np.frombuffer(s, dtype="int32", count=ndim, offset=8)

    # If the array has less than three dimensions, three int32 are still used to
    # save the shape info (remaining int32 are simply set to1). The shape info
    # hence uses max(3, ndim) bytes.
    bytes_used_for_shape_info = max(3, ndim) * 4

    # The remaining bytes are the array.
    data = np.frombuffer(s, dtype=dtype, offset=8 + bytes_used_for_shape_info)
